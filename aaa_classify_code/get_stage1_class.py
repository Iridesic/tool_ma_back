...
def predict_from_csv(csv_file_path, model_path, training_feature_path, history_days=50):
    ...
    try:
        ...
        if stage1_df.empty:
            raise ValueError("CSV文件中无stage=1的数据，无需预测")
        
        # 关键修改：以stage=1的最小日期作为基准推历史（而非整个CSV的最小日期）
        stage1_min_date = stage1_df['timestamps'].min()  # stage=1数据的起始日期
        end_date = current_df['timestamps'].max()        # 整个CSV的结束日期（不变）
        history_start_date = stage1_min_date - pd.Timedelta(days=history_days)  # 仅为stage=1推历史
...
def find_source_csv_with_history(start_date, end_date, input_csv_path):  # 新增input_csv_path参数
    ...
    default_path = r'D:\self\code\ma_final\data\stage1_class_predict.csv'
...
if __name__ == "__main__":
    # 短数据检测
    # csv_file = r'D:\self\code\ma_final\data\stage1_class_predict.csv'
    # 长数据检测
    csv_file = r'D:\self\code\ma_final\data\000021_fragment_1_20220803_to_20220809_extended.csv'
    try:
