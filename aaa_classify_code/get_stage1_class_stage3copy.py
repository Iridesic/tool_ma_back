...
import pandas as pd
import os
from aaa_classify_code.setter_stage3 import prepare_data_for_gp
import chardet
import shutil
...
def predict_from_csv(csv_file_path, model_path, training_feature_path, history_days=50):
    ...
    try:
        ...
        stage3_df = current_df[current_df['stage'] == 3].copy()
        if stage3_df.empty:
            raise ValueError("CSV文件中无stage=3的数据，无需预测")
        
        # 关键修改：以stage=3的最小日期作为基准推历史
        stage3_min_date = stage3_df['timestamps'].min()  # stage=3数据的起始日期
        end_date = current_df['timestamps'].max()        # 整个CSV的结束日期（不变）
        history_start_date = stage3_min_date - pd.Timedelta(days=history_days)  # 仅为stage=3推历史

        # 调用修改后的find_source_csv_with_history，传入当前CSV路径
...
def find_source_csv_with_history(start_date, end_date, input_csv_path):  # 新增input_csv_path参数
    """
    查找包含指定时间范围的源CSV文件
    ...
    default_path = r'D:\self\code\ma_final\data\stage3_class_predict.csv'
...
if __name__ == "__main__":
    # 长数据检测
    # csv_file = r'D:\self\code\ma_final\data\stage3_class_predict.csv'
    # 短数据检测
    csv_file = r'D:\self\code\ma_final\data\000021_fragment_2_20220819_to_20220824_extended.csv'
    try:
        ...
        if stage3_count > 10:
            model_path = r'D:\self\code\ma_final\model_new\stage3_long\stock_classifier_pipeline.pkl'
            training_feature_path = r'D:\self\code\ma_final\model_new\stage3_long\training_feature_columns.csv'
            print("使用模型路径:", model_path)
        else:
            model_path = r'D:\self\code\ma_final\model_new\stage3_short\stock_classifier_pipeline.pkl'
            training_feature_path = r'D:\self\code\ma_final\model_new\stage3_short\training_feature_columns.csv'
            print("使用模型路径:", model_path)
